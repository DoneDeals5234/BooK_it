...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { playerIds, subscriptionIds, title, body: messageBody, data: dataIn } = body;

    console.log("📱 FCM notification request (send-notification wrapper):", { title, playerIdsCount: playerIds?.length });
    ...
    // Map player_id to fcm_token (compatibility layer)
    if (playerIds?.length > 0) {
      ...
      if (devices) fcmTokens = [...new Set(devices.map((d: any) => d.fcm_token))];
    }

    if (fcmTokens.length === 0) {
      console.warn("⚠️ No FCM tokens found for the provided playerIds.");
